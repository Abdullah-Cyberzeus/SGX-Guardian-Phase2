...
- **Admin recovery — known gap, undocumented feature, not a bug fix.** No in-app password-reset/recovery flow exists. The only owner-account bootstrap path is `POST /auth/signup`, which is permanently locked (`create_initial_owner` in `src/api/auth/store.rs`) the moment the first owner user is created — by design, it cannot be re-run as a reset mechanism. Accounts live in a plain `users.json` (`src/api/auth/store.rs::USERS_FILE`). A locked-out admin today has no supported recovery path short of physical/CLI access to edit or remove that file and restart the daemon. This is flagged as a real gap, not fixed here — it needs its own design (e.g. a signed recovery token workflow) and security review before being built.
- **TLS trust.** A self-signed certificate with `guardian.local` in its SAN list is generated at every boot (`src/main.rs`, `src/tls.rs`) and loaded with enforced `0600` key permissions (`src/api/tls.rs`). The resulting first-visit browser warning is expected for this device class (same as most local router/NAS admin consoles) — no interactive trust/pinning UI was built or is planned; task 4 is satisfied by this existing setup.
- **Member/admin shell coexistence — confirmed safe, no change needed.** One router (`frontend/src/app/routes.ts`), one service worker (`frontend/public/sw.js`) registered once at root scope. The worker's `isApi` check excludes all `/api/*` traffic from both precaching and the fetch handler, so authenticated responses are never cached for either role.
- **`VITE_API_URL`.** Leave unset in production Admin Console builds — the client already defaults to same-origin `/api/v1` (`frontend/src/app/services/api.ts`). The build now warns (not fails) if it's set to a non-local host, since the same variable is a legitimate escape hatch for local dev tunneling (see the `ngrok` header handling in the same file).
- **Audit logging fast-follow.** `src/api/handlers/devices.rs` (the task's explicit example — pair/add/edit/remove/block/reject/unblock/unpair), `crl.rs` (revoke/unrevoke), `policy.rs` (save/sign-deploy), `geofence.rs` (zone create/edit/delete, actions), and `backup.rs`/`restore.rs` (create/import/delete/restore/apply/undo) all now call `log_audit`. No other REST handler file was found with an equally complete absence of audit calls.
- **Additional mock-fallback findings beyond the original inventory**, fixed in this pass:
  - `DV03DeviceDetail.tsx` / `DV06SecurityScan.tsx` / `STTopology.tsx` (Settings) were entirely mock-shaped and unreachable from any real navigation — `DV01DevicesList.tsx` and the live topology (`CircleLiveTopology`/`HM03NetworkTopology`) already implement the real equivalents. Replaced with a pointer back to the real screen rather than deleted, since the routes are still directly reachable.
  - `ST07Geofencing.tsx` (Settings) was the same pattern — non-functional Add/Delete buttons, hardcoded fake location — while a full, real geofence zone management UI already exists in `EnterpriseTopology.tsx` (home topology), backed by the real `geofence.rs` API. Pointed at the real screen.
  - `frontend/src/app/hooks/useApiData.ts`'s `useDevices()` hook called a `deviceService.getAll()` method that does not exist (a `deviceService.ts`/`managedDeviceService.ts` split had happened without updating this hook) — this was a genuine runtime bug, not just an unnecessary fallback. Removed along with its only (now-dead) callers.
  - `AppSidebar.tsx`'s Alerts and Devices nav badges were computed once from static mock arrays at module load and never updated. Alerts badge now derives from `useAlerts()`; the Devices "pending" badge had no real backend concept to map to, so it was set to `0` rather than left fabricated. The sidebar's own online/offline status dot now reflects `useGuardianConnectivity()` instead of a hardcoded `mockGuardian.status`.
  - `NW01CirclesList.tsx`'s inline circle detail panel seeded its Chat/Calls tabs from `mockCircles` data keyed by nothing (`mockCircles.find(...) || mockCircles[0]`, effectively always the first mock circle) whenever the real circle object had no `messages`/`calls` fields — which the real Circles API never populates (messaging goes through the WebSocket chat system). The mock seed was removed so these tabs are empty rather than showing fabricated conversation history. **Known remaining gap, not fixed here**: the panel's own `sendMessage`/call-start actions only update local component state and never call a real API — this embedded mini-chat/calls UI needs the same kind of real-wiring pass `ChatConversationScreen.tsx` already has, or should be replaced with a pointer to it, matching the pattern used elsewhere in this Phase 10 pass. Flagged for a follow-up, not attempted here given its size.
- **Connectivity gating (task 9).** Applied `disabled={!reachable}` (via `useGuardianConnectivity()`) plus an explanatory `title` to the state-changing controls in `ST16PendingApprovals.tsx` (approve/reject) and `CrlOperationsPanel.tsx` (gossip trigger, emergency re-broadcast, offline sync). The existing global `OfflineBanner` (`MainLayout.tsx`) already covers the "cached data may be stale" exit criterion app-wide — no new per-screen staleness badge was added. Device pair/block/unblock/remove and key-management mutation surfaces were not covered in this pass and remain a follow-up if broader connectivity-gating coverage is wanted.
