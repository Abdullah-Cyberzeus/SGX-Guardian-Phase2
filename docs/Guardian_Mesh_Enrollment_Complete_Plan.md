...
| P1.2 | **Split `main()` into two stages.** **Stage A `boot_local()`** covers everything that does not need a circle: filesystem bootstrap, KeyManager/SE050/DIK/tamper, DID create, PCR status, local policy enforcement, admin TLS cert (`tls::ensure_node_certificate_or_generate`, already independent of Nebula), runtime/Wi-Fi daemon, `AppState`, and **REST API on :8443**. **Stage B `mesh::activation::activate_mesh(profile)`** takes the current code at `main.rs:807-2360` (Nebula install checks, overlay, lighthouse registry, registry sync, CA servers, DID publish, mTLS gRPC server, CRL gossip/offline, circle snapshot, xfer, chat gRPC, pings). Stage B runs when lifecycle reaches `CIRCLE_MEMBER`, immediately at boot if already enrolled. Move code; do not rewrite it | `src/main.rs` → `src/startup/local.rs` (new), `src/mesh/activation.rs` | L |
| P1.3 | **Remove fatal exits from the enrollment path.** Replace `std::process::exit(1)` in the member branch (`main.rs:1256-1308`) and Nebula binary checks with `lifecycle.fail(reason)`. The UI shows the error with retry | `src/mesh/activation.rs` | S |
| P1.4 | **Late-bound dependencies in `AppState`.** `did::Resolver.ca_host` and anything else built from `did_doc_publish_state` must be updatable after activation. Wrap them in `ArcSwap` and add `Resolver::set_ca_host()`. Review `AppState::from_env` for fields that assume a circle | `src/did/resolver.rs`, `src/api/state.rs` | M |
| P1.5 | **API gating middleware.** Route groups are tagged `Provisioning` (always on: auth, node/status, boot-status, wifi, `/api/v1/mesh/*`, logs, PCR, DID self) or `MeshRequired` (peers, chat, call, group_call, xfer, circles (comms), crl, relay, transport, vc, vault-sharing…). While not `ONLINE`, `MeshRequired` returns **409 `{code:"GUARDIAN_NOT_ENROLLED", lifecycle:"UNENROLLED"}`** | `src/api/mod.rs`, `src/api/routes.rs`, new `src/api/mesh_gate.rs` | M |
...
| P1.7 | **Frontend lifecycle gate.** New `MeshLifecycleContext` (poll + WS). `SYS02SplashScreen` flow: no admin → onboarding account setup. Admin exists and state ≠ ONLINE → `/setup`. ONLINE → home. `ProtectedRoute` for mesh screens redirects to `/setup` on 409 | `frontend/src/app/contexts/MeshLifecycleContext.tsx`, `screens/system/SYS02SplashScreen.tsx`, `components/ProtectedRoute.tsx`, `routes.ts` | M |
| P1.8 | **SU01 Setup Choice screen** (spec §32 "Initial state"). Status card showing `UNENROLLED`, the Guardian ID, and DID/hardware badges. Two actions: **Create Mesh Circle** and **Join Existing Circle** | `frontend/src/app/screens/setup/SU01SetupChoice.tsx` | S |
| P1.9 | **Guardian ID choice.** For a fresh unit the CLI arg becomes optional: `sgx-guardian [guardian_id]`. If absent, derive `gx-<first 8 of DID hash>` and let the operator rename it in SU01 before enrollment. Keep the argument for legacy and systemd units | `src/main.rs`, `packaging/sgx-guardian.service` | S |

**Phase 1 exit criteria**
- A fresh Guardian with an empty `/var/lib/sgx-guardian` serves `https://<lan>:8443` in under 30 s, shows SU01, and the daemon stays up indefinitely without a circle.
- An already-enrolled Guardian boots straight to ONLINE with identical behaviour to Phase 0, verified by the full regression suite.
- Unplugging the CA during member boot leads to `ERROR` with a readable reason, not a crash loop under systemd.
...
| ID | Task | Files | Size |
|---|---|---|---|
| P2.1 | **CA initialisation service.** `mesh::ca::create_circle(name, options)`: generate `circle_id = "circle-" + 12 random base32` and overlay CIDR (default `192.168.100.0/24`, optionally a user-chosen `/16`–`/24`). Run `nebula-cert ca -name "<circle_name> CA" -duration <policy>` (**replaces the constant `guardian-circle-ca`**). Issue its own cert via local keygen + `-in-pub`, generate the PA key (`policy_authority::PaKey::load_or_generate`, formerly nodeA-only at `main.rs:119`), create the `Circle{kind: Mesh}` record and **self-issued owner VC**, write the `MeshProfile{role: Ca}`, then go to `CIRCLE_MEMBER` → activate | `src/mesh/ca/mod.rs`, `src/nebula/ca.rs`, `src/circle/store.rs`, `src/policy_authority.rs` | M |
| P2.2 | **Crash safety.** Creation is a staged transaction under `mesh/staging/create-<ts>/`. Only the final rename commits it. On boot, stale staging directories are cleaned up. This replaces the "partial CA state → manual intervention" error in `ca.rs:40` | `src/mesh/ca/mod.rs` | S |
...
| P2.4 | **API.** `POST /api/v1/mesh/circles` `{name, overlay_cidr?, policy?}` → 202 + lifecycle transitions. `GET /api/v1/mesh/circle` returns the current circle (both roles) | `src/api/handlers/mesh.rs` | S |
| P2.5 | **Frontend SU02 Create Circle** (spec §32). Name, advanced options (subnet, validity, attestation mode), progress steps (CA key → own cert → policy → mesh online), then a success screen showing the **CA fingerprint** in words + hex, with a **"Invite a Guardian"** CTA leading to P4.8 | `screens/setup/SU02CreateCircle.tsx`, `services/meshService.ts` | M |
| P2.6 | **Keep Comms Circles separate.** The existing `OB08CreateFirstCircle` / `NW02CreateCircle` create **Comms** circles. Relabel the UI to "Comms Circle" vs "Mesh Circle" so operators do not confuse them. Mesh circle creation happens only in `/setup` | `screens/onboarding/OB08…`, `screens/network/NW0*` | S |
