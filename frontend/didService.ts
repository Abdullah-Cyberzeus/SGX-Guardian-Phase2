...
export function OB06AccountSetup() {
  ...
  const handleCylenium = () => {
    ...
    // Simulate OAuth; after 1.8s show "new user" success
    setTimeout(() => {
      // Deterministic demo: pick based on a simple toggle
      const scenario = localStorage.getItem("sgx_cylenium_scenario") || "new";
      if (scenario === "enterprise") {
        setCyleniumState("success-enterprise");
      } else if (scenario === "fail") {
        setCyleniumState("failed");
        toast.error("Cylenium connection failed. Try again or use email.");
      } else {
        setCyleniumState("success-new");
        // After 2s auto-advance
        setTimeout(() => {
