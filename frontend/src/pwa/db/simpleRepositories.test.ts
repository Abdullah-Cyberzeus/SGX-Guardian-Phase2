...
describe("fileRepository", () => {
  it("saves and lists files", async () => {
    await fileRepository.save({ id: "f1", name: "report.pdf", mime: "application/pdf", size: 1024, updatedAt: Date.now() });
    expect(await fileRepository.list()).toHaveLength(1);
  });

  it("replaceAll clears the store before writing the new set", async () => {
    await fileRepository.save({ id: "stale", name: "old.txt", mime: "text/plain", size: 1, updatedAt: 1 });
    await fileRepository.replaceAll([{ id: "f1", name: "new.txt", mime: "text/plain", size: 2, updatedAt: 2 }]);
