...
export function NW02CreateCircle() {
  ...
  const [visualMemberDid, setVisualMemberDid] = useState("");
  ...
  const handleQuickCreate = async () => {
    ...
    setLoading(true);
    try {
      const created = await circleService.create({ name: name.trim(), description: description.trim() });
      toast.success("Circle created");
      navigate(`/network?circle=${encodeURIComponent(created.id)}`, { replace: true });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Circle creation failed");
      setLoading(false);
    }
  };
  ...
  const addVisualMemberByDid = () => {
    const did = visualMemberDid.trim();
    if (!did.startsWith("did:") || members.some((member) => member.did === did)) return;
    const label = did.split(":").pop() || "Member";
    handleAddMember({ id: did, did, name: label, initials: label.slice(0, 2).toUpperCase(), online: false });
    setVisualMemberDid("");
  };
  ...
  const handleVisualCreate = async () => {
    if (!canCreate) return;
    setVisualLoading(true);
    try {
      const created = await circleService.create({ name: circleName.trim() });
      const results = await Promise.allSettled(members.map((member) => circleService.addMember(created.id, { did: member.did, role: "member" })));
      const failures = results.filter((result) => result.status === "rejected").length;
      if (failures) toast.error(`Circle created, but ${failures} member${failures === 1 ? "" : "s"} could not be added`);
      setVisualLoading(false);
      setVisualDone(true);
      setTimeout(() => navigate(`/network?circle=${encodeURIComponent(created.id)}`, { replace: true }), 900);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Circle creation failed");
      setVisualLoading(false);
    }
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-5">
                <label className="mb-2 block text-xs font-medium" style={{ color: "var(--muted-foreground)" }}>Guardian DID</label>
                <input value={visualMemberDid} onChange={(e) => setVisualMemberDid(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addVisualMemberByDid()} placeholder="did:guardian:…" className="w-full rounded-md border border-border px-3 outline-none" style={{ height: "46px", backgroundColor: "var(--input-background)", color: "var(--foreground)" }} />
                <p className="mt-2 text-xs leading-5" style={{ color: "var(--muted-foreground)" }}>Enter a resolvable Guardian DID. Membership credentials are issued only after the Circle is created.</p>
                <button onClick={addVisualMemberByDid} disabled={!visualMemberDid.trim().startsWith("did:")} className="mt-4 w-full flex items-center justify-center gap-2 rounded-md" style={{ height: "44px", backgroundColor: visualMemberDid.trim().startsWith("did:") ? "var(--primary)" : "var(--muted)", color: visualMemberDid.trim().startsWith("did:") ? "var(--primary-foreground)" : "var(--muted-foreground)", border: "none" }}><UserPlus size={16} />Add to visual Circle</button>
              </div>
            </div>
          </div>
        )}
