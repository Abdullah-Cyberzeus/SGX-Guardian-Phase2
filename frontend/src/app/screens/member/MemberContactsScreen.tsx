...
import { CalendarDays, Check, Loader2, Pencil, Plus, Search, ShieldCheck, Smartphone, Trash2, UserRound, Users, X } from "lucide-react";
...
export function MemberContactsScreen() {
  ...
  const contacts = useMemo(() => {
    ...
    return needle ? all.filter((peer: Peer) => `${contactNameForDid(peer.did) || ""} ${peer.displayName} ${peer.fullName || ""} ${peer.deviceName} ${peer.role} ${peer.peerId} ${peer.did}`.toLowerCase().includes(needle)) : all;
  }, [data, cached, query, contactNameForDid]);
  ...
  return <div className="flex h-full flex-col">
    ...
    <div className="flex-1 overflow-y-auto"><div className="mx-auto max-w-3xl divide-y divide-border">
      ...
      {contacts.map((peer: Peer) => {
        const displayName = contactNameForDid(peer.did) || peer.displayName || peer.peerId;
        return <article key={peer.did} className="flex items-center gap-3 px-4 py-4 md:px-6">
        ...
        <button type="button" onClick={() => setSelectedDid(peer.did!)} className="min-w-0 flex-1 text-left">
          <div className="flex items-center gap-2"><p className="truncate text-sm font-semibold">{displayName}</p><ShieldCheck size={14} className="text-primary" /></div>
    ...
    {selected && <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm" onClick={() => setSelectedDid(null)}>
      <aside className="ml-auto flex h-full w-full max-w-sm flex-col border-l border-border bg-card shadow-xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border p-4">
          <div className="min-w-0"><p className="truncate text-base font-semibold">{contactNameForDid(selected.did) || selected.displayName || selected.peerId}</p><p className="truncate text-xs text-muted-foreground">{selected.did}</p></div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div className="min-w-0"><p className="truncate text-sm font-semibold">{contactNameForDid(selected.did) || selected.fullName || selected.displayName}</p><p className="text-xs text-muted-foreground">{presenceHidden(selected) ? "Presence hidden" : selected.presenceStale ? "Presence stale" : selected.online ? "Online" : `Offline · ${selected.lastSeenAgo}`}</p></div>
          </div>
          <dl className="grid gap-3 text-sm">
            ...
            <div className="flex items-center justify-between gap-4 border-b border-border pb-2"><dt className="text-muted-foreground">IP Address</dt><dd className="truncate font-mono text-xs font-medium">{selected.ip || selected.overlayIp || selected.physicalIp || "Not available"}</dd></div>
