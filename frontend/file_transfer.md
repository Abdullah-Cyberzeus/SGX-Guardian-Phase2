...
| 100| POST | `/api/v1/xfer/send` | Send a file to a peer DID using Secure XFER |
| 101| GET | `/api/v1/xfer/transfers` | List Secure XFER transfers and transfer counters |
| 102| GET | `/api/v1/xfer/transfers/{id}` | Get details for one Secure XFER transfer |
| 103| POST | `/api/v1/xfer/transfers/{id}/cancel` | Cancel one Secure XFER transfer |
| 104| GET | `/api/v1/xfer/inbox` | List received Secure XFER inbox files |
| 105| GET | `/api/v1/vault/files` | List encrypted Vault files |
| 106| GET | `/api/v1/vault/files/{id}` | Get encrypted Vault file metadata |
| 107| PATCH | `/api/v1/vault/files/{id}` | Rename or move an encrypted Vault file |
| 108| DELETE | `/api/v1/vault/files/{id}` | Delete an encrypted Vault file |
| 109| GET | `/api/v1/vault/files/{id}/download` | Download decrypted Vault file content |
| 110| GET | `/api/v1/vault/files/{id}/preview` | Preview decrypted Vault file content |
| 111| POST | `/api/v1/vault/upload` | Upload a file into encrypted Vault storage |
| 112| POST | `/api/v1/vault/folders` | Create an encrypted Vault folder |
| 113| GET | `/api/v1/vault/quota` | Get encrypted Vault quota usage |
| 114| GET | `/api/v1/vault/overview` | Get encrypted Vault storage overview |
| 115| GET | `/api/v1/vault/search` | Search encrypted Vault files |
| 116| GET | `/api/v1/vault/folders` | Not implemented in current Rust source |
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/api/v1/xfer/send` | Send a file to a peer DID using Secure XFER |
| GET | `/api/v1/xfer/transfers` | List Secure XFER transfers and transfer counters |
| GET | `/api/v1/xfer/transfers/{id}` | Get details for one Secure XFER transfer |
| POST | `/api/v1/xfer/transfers/{id}/cancel` | Cancel one Secure XFER transfer |
| GET | `/api/v1/xfer/inbox` | List received Secure XFER inbox files |
| GET | `/api/v1/vault/files` | List encrypted Vault files |
| GET | `/api/v1/vault/files/{id}` | Get encrypted Vault file metadata |
| PATCH | `/api/v1/vault/files/{id}` | Rename or move an encrypted Vault file |
| DELETE | `/api/v1/vault/files/{id}` | Delete an encrypted Vault file |
| GET | `/api/v1/vault/files/{id}/download` | Download decrypted Vault file content |
| GET | `/api/v1/vault/files/{id}/preview` | Preview decrypted Vault file content |
| POST | `/api/v1/vault/upload` | Upload a file into encrypted Vault storage |
| POST | `/api/v1/vault/folders` | Create an encrypted Vault folder |
| GET | `/api/v1/vault/quota` | Get encrypted Vault quota usage |
| GET | `/api/v1/vault/overview` | Get encrypted Vault storage overview |
| GET | `/api/v1/vault/search` | Search encrypted Vault files |
| GET | `/api/v1/vault/folders` | Not implemented in current Rust source |
---
...
### 6.5 POST `/api/v1/xfer/send`

- Handler: `handlers::xfer::send`
- Purpose: Queue an outbound Secure XFER file transfer to a peer DID.
- Path parameters: none
- Query parameters: none
- Request body: JSON `SendRequest`
...
- Success: `200 OK`, JSON `SendResponse`
...
- Errors: XFER error mapping in section 6.3.

### 6.6 GET `/api/v1/xfer/transfers`

- Handler: `handlers::xfer::list`
- Purpose: Return outbound Secure XFER transfer history and aggregate transfer counters.
- Path parameters: none
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `TransferListResponse`
...
```json
{
  ...
  "bytes_transferred": 0,
  "last_transfer": "LastTransfer or null"
}
```

- Errors: XFER error mapping in section 6.3.

### 6.7 GET `/api/v1/xfer/transfers/{id}`

- Handler: `handlers::xfer::detail`
- Purpose: Return detailed status for one Secure XFER transfer.
- Path parameters: `id` string
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `TransferSummary`
- Errors: `400 BAD_REQUEST` for empty id, `404 NOT_FOUND` for missing transfer, plus XFER error mapping in section 6.3.

### 6.8 POST `/api/v1/xfer/transfers/{id}/cancel`

- Handler: `handlers::xfer::cancel`
- Purpose: Cancel a queued or active Secure XFER transfer.
- Path parameters: `id` string
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `CancelResponse`
...
- Errors: `400 BAD_REQUEST` for empty id, `404 NOT_FOUND` when `cancel_transfer` returns false, plus XFER error mapping in section 6.3.

### 6.9 GET `/api/v1/xfer/inbox`

- Handler: `handlers::xfer::inbox`
- Purpose: List inbound Secure XFER files received by this node.
- Path parameters: none
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `InboxResponse`
  ...
  "files": [
    {
      ...
      "completed_at": "RFC3339 string or null",
      "file_sha256": "string",
      "vault_id": "string or null",
      "download_path": "string or null",
...
### 6.10 GET `/api/v1/vault/files`

- Handler: `handlers::vault::list`
- Purpose: List encrypted Vault file metadata records.
- Path parameters: none
- Query parameters: `circle_id` optional string, `namespace` optional string, `folder_id` optional string, `starred` optional boolean
- Request body: none
- Success: `200 OK`, JSON `VaultListResponse`
...
- Errors: Vault error mapping in section 6.4.

### 6.11 GET `/api/v1/vault/files/{id}`

- Handler: `handlers::vault::detail`
- Purpose: Return metadata for one encrypted Vault file.
- Path parameters: `id` string
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `VaultRecord`
- Errors: `404 NOT_FOUND` when no record exists for `id`, plus Vault error mapping in section 6.4.

### 6.12 PATCH `/api/v1/vault/files/{id}`

- Handler: `handlers::vault::rename_or_move_file`
- Purpose: Rename an encrypted Vault file or move it between folders.
- Path parameters: `id` string
- Query parameters: none
- Request body: JSON `UpdateFileRequest`
...
- Success: `200 OK`, JSON `VaultRecord`
- Errors: `400 BAD_REQUEST` when neither field changes or filename is empty, `404 NOT_FOUND` for missing file or folder, plus Vault error mapping in section 6.4.

### 6.13 DELETE `/api/v1/vault/files/{id}`

- Handler: `handlers::vault::delete_file`
- Purpose: Delete an encrypted Vault file.
- Path parameters: `id` string
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `VaultActionResponse`

```json
{
  "success": true,
  "message": "deleted <vault_id>"
}
```

- Errors: `404 NOT_FOUND` when no record exists for `id`, plus Vault error mapping in section 6.4.

### 6.14 GET `/api/v1/vault/files/{id}/download`

- Handler: `handlers::vault::download`
- Purpose: Download decrypted Vault file content as an attachment.
- Path parameters: `id` string
- Query parameters: none
- Request body: none
- Success: `200 OK`, streamed bytes
- Success headers: `Content-Type: <record.mime>`, `Content-Length: <record.size_plain>`, `Content-Disposition: attachment; filename="<sanitized filename>"`, `x-sgx-vault-id: <vault_id>`, `x-sgx-vault-download: /api/v1/vault/files/{id}/download`
- Errors: `404 NOT_FOUND` when no record exists for `id`, plus Vault error mapping in section 6.4.

### 6.15 GET `/api/v1/vault/files/{id}/preview`

- Handler: `handlers::vault::preview`
- Purpose: Preview decrypted Vault file content inline when the file type is supported.
- Path parameters: `id` string
- Query parameters: none
- Request body: none
- Stream success: `200 OK`, streamed bytes with `Content-Disposition: inline; filename="<sanitized filename>"`
- Metadata success: `200 OK`, JSON `VaultPreviewMetadata`

```json
{
  "previewable": false,
  "reason": "preview disabled for large file | preview not available for this content type",
  "mime": "string",
  "size_plain": 0
}
```

- Previewable when `size_plain <= 33554432` and MIME starts with `image/`, starts with `text/`, or equals `application/pdf`, `application/json`, `application/xml`, or `text/csv`.
- Errors: `404 NOT_FOUND` when no record exists for `id`, plus Vault error mapping in section 6.4.

### 6.16 POST `/api/v1/vault/upload`

- Handler: `handlers::vault::upload`
- Purpose: Upload and store a local file in encrypted Vault storage.
- Path parameters: none
- Query parameters: `ns` optional string, `folder_id` optional string
- Request body: `multipart/form-data`; handler uses the first field with a filename or field name `file`
- Success: `200 OK`, JSON `VaultUploadResponse`
...
- Errors: `400 BAD_REQUEST` for invalid multipart or missing file field, `404 NOT_FOUND` for missing folder, `413 PAYLOAD_TOO_LARGE` for quota, plus Vault error mapping in section 6.4.

### 6.17 GET `/api/v1/vault/folders`

- Handler: none in current Rust source
- Purpose: Not implemented in current Rust source.
- Path parameters: none
- Query parameters: none documented because no handler exists
- Request body: none documented because no handler exists
- Success response: none documented because no route/handler exists
- Error responses and status codes: no handler-level mapping exists. The path is registered only for `POST`, so `GET` is a router method mismatch rather than a Vault handler response.

### 6.18 POST `/api/v1/vault/folders`

- Handler: `handlers::vault::create_folder`
- Purpose: Create an encrypted Vault folder.
- Path parameters: none
- Query parameters: none
- Request body: JSON `CreateFolderRequest`
...
- Success: `200 OK`, JSON `FolderNode`
- Errors: `400 BAD_REQUEST` for invalid namespace/folder/name structure, `404 NOT_FOUND` for missing parent folder, `409 CONFLICT` for duplicate folder name, plus Vault error mapping in section 6.4.

### 6.19 GET `/api/v1/vault/quota`

- Handler: `handlers::vault::quota_status`
- Purpose: Return encrypted Vault quota usage.
- Path parameters: none
- Query parameters: `ns` optional string, `circle_id` optional string
- Request body: none
- Success: `200 OK`, JSON `VaultQuotaResponse`
...
### 6.20 GET `/api/v1/vault/overview`

- Handler: `handlers::vault::overview`
- Purpose: Return encrypted Vault storage overview and namespace usage.
- Path parameters: none
- Query parameters: none
- Request body: none
- Success: `200 OK`, JSON `VaultOverviewResponse`
...
### 6.21 GET `/api/v1/vault/search`

- Handler: `handlers::vault::search`
- Purpose: Search encrypted Vault files by filename.
- Path parameters: none
- Query parameters: `q` required string, `limit` optional usize capped at 100 and defaulting to 50
- Request body: none
- Success: `200 OK`, JSON `VaultSearchResponse`
...
- Errors: `400 BAD_REQUEST` for empty `q`, plus Vault error mapping in section 6.4.
...
Source-truth note: 11 of the 12 requested Vault paths are registered in `src/api/routes.rs`; `GET /api/v1/vault/folders` is not registered in the current Rust source.

## 7. New Secure File/Image APIs

Existing API reused; no new API required.

No new route, request model, response model, storage format, XFER protocol behavior, CRL sender-validation logic, or Vault wrapping/encryption behavior is required for this documentation task.