...
impl InterfaceDetector {
    ...
    pub fn detect_all() -> CotResult<Vec<InterfaceInfo>> {
        ...
        for nic in nic_list {
            ...
            // Use real Linux operstate for link detection
            let link_status = Self::read_linux_operstate(&nic.name);

            // Combine operstate + IP presence for usable status
            let status = match (link_status, ip_addr.is_some()) {
                (InterfaceStatus::Up, true) => InterfaceStatus::Up,
                (InterfaceStatus::Down, _) => InterfaceStatus::Down,
                (InterfaceStatus::Unknown, _) => InterfaceStatus::Unknown,
                (_, false) => InterfaceStatus::Down,
            };
