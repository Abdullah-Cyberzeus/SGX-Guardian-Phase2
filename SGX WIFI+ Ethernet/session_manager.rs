...
pub struct SessionManager {
    sessions: Arc<RwLock<HashMap<(String, TransportType), Session>>>,
    timeout_secs: i64,
}
...
impl SessionManager {
    ...
    pub async fn get_or_create(
        ...
        let key = (remote_device_id.to_string(), transport);
    ...
    pub async fn migrate_session(
        ...
        // Find existing session for this remote device
        let existing_key = sessions
            .keys()
            .find(|(id, _)| id == remote_device_id)
            .cloned();

        if let Some((_, old_transport)) = existing_key {
            let old_key = (remote_device_id.to_string(), old_transport);

            if let Some(mut session) = sessions.remove(&old_key) {
                session.migrate_transport(new_transport);
                let new_key = (remote_device_id.to_string(), new_transport);
                sessions.insert(new_key.clone(), session.clone());
                return Ok(session);
            }
        }

        Err(CotError::SessionInvalid(format!(
            "No session for {}",
            remote_device_id
        )))
    }
    ...
    pub async fn get_session(&self, remote_device_id: &str) -> Option<Session> {
        ...
        sessions
            .iter()
            .find(|((id, _), _)| id == remote_device_id)
            .map(|(_, s)| s.clone())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_separate_transport_sessions() {
        let mgr = SessionManager::new();
