...
pub async fn run(mut rx: Receiver<String>) -> Result<()> {
    ...
    if let Ok(contents) = contents {
        ...
        if let Ok(peers_list) = parsed {
            for peer in peers_list {
                let ip_part = peer.peer_id.split(':').next().unwrap_or("");

                if !crate::dynamic_config::is_routable_ip(ip_part) {
                    println!(
                        "⚠️ Skipping persisted peer {} — non-routable IP",
                        peer.peer_id
                    );
                    continue;
                }
    ...
    let node_conf = load_config(&format!("/etc/sgx-guardian/config/{}.yaml", node_id_env))
            ...
            if let Ok(contents) = contents {
                ...
                if let Ok(peers_list) = parsed {
                    for peer in peers_list {
                        ...
                        if !crate::dynamic_config::is_routable_ip(ip_part) {
                            println!(
                                "⚠️ Skipping peer {} during re-attestation — non-routable IP",
                                peer.peer_id
                            );
                            continue;
                        }
