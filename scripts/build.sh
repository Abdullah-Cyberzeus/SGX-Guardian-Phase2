...
build_workspace() {
  ...
  rm -f \
    ...
    "${out_dir}/sgx-pa-cli" \
    "${ARTIFACT_DIR}/sgx_guardian_client" \
...
collect_artifacts() {
  ...
  # Keep artifact names exactly same as target/release binary names.
  cp "${daemon_src}" "${ARTIFACT_DIR}/sgx_guardian_client"
  cp "${cli_src}" "${ARTIFACT_DIR}/sgx-pa-cli"

  log "Artifacts ready"
  ls -lh "${ARTIFACT_DIR}/sgx_guardian_client" "${ARTIFACT_DIR}/sgx-pa-cli"
  file "${ARTIFACT_DIR}/sgx_guardian_client" "${ARTIFACT_DIR}/sgx-pa-cli"

  log "SHA256"
  sha256sum "${ARTIFACT_DIR}/sgx_guardian_client" "${ARTIFACT_DIR}/sgx-pa-cli"
}
