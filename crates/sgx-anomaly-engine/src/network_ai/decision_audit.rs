...
pub fn persist_decision_audit(
    ...
    if let Some(reward) = &record.selected_reward {
        append_jsonl(
            &paths.rewards_jsonl,
            &serde_json::json!({
...
mod tests {
    ...
    use crate::network_ai::{
        EligibilityDecision, EligibilityReason, NetworkAiFeatureVector, RouteCandidateInventory,
        RoutePrediction, RouteQualityComponents, TrafficClass,
    };
    ...
    fn persists_current_and_linked_append_only_audit_chain() {
        ...
        assert_eq!(
            fs::read_to_string(&paths.rewards_jsonl)
                .unwrap()
                .lines()
                .count(),
            2
