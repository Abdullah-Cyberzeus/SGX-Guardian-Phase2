...
**Environment:** Docker container cohort (WSL2) | **Date:** 2026-07-21 | **Tester:** Aliza Malik
...
- This file records **only confirmed PASS** steps.
- Failed, blocked, or inconclusive commands are intentionally **not recorded** here.
- Whenever a new requirement passes, its exact command, expected result, observed result, and verdict get appended/updated right here.
...
- All commands are run from the **host terminal**.
- Preferred pattern for REST verification:
  - `docker exec sgx-nodeA sh -lc 'curl -s http://127.0.0.1:8443/api/v1/...'`
- For file reads, use host-side JSON parsing:
  - `docker exec sgx-nodeA sh -lc 'cat /path/file.json' | python3 -m json.tool`
- `docker exec` always runs from the host shell; don't run `docker exec` from inside a container.
- In this current run, `nodeB` is mapped to host port `28444`.
- For member-management tests, don't use a fake string DID unless it's a valid base58 Guardian DID.
  - Safe choice: `TEST_MEMBER_DID="$NODEC_DID"`
- For VC verification use `GET /api/v1/vc/show?scope=issued|own|peers`.
  - `GET /api/v1/vc/list` only returns local `own` VCs; `scope=issued` doesn't apply on this route.
- For the mesh-isolation runtime check, `GET /api/v1/crl/gossip/status` is the authoritative signal.
  - `GET /api/v1/vc/summary` shows the last local own VC; it is not the gossip runtime circle resolver.
...
**Verdict:** PASS - Mesh Circle seeding, comms Circle creation, and signed registry persistence verified in the Docker cohort.
...
**Verdict:** PASS - circle rename, restart persistence, and signed on-disk registry update verified in the Docker cohort.
...
**Verdict:** PASS - membership VC issuance, members listing, and issued VC visibility verified in the Docker cohort.
...
**Verdict:** PASS - the role-change admin flow and the owner-only DID/VC authorization gate verified in the Docker cohort.

---

## ✅ Requirement 5 - Signed invite tokens (expired / tampered / replayed / non-owner rejected)

> Invite token signed (ECDSA-P256), time-limited (default 24h, clamped 5min..30d), replay-protected (`max_uses`, default 1). Tampered/expired/replayed/non-owner-minted tokens must be **rejected**. Permissions are never blindly taken from the token — they're re-derived and validated (no `vc:issue` escalation).
...
Tampered join/preview → rejected with HTTP 400 and an invalid invite token message
```
...
**Verdict:** PASS - signed invite token tamper rejection verified in the Docker cohort.
...
**Verdict:** PASS - QR/share payload, invite listing, and the invite-revoke flow verified in the Docker cohort.
...
**Verdict:** PASS - preview, redeem, nodeB VC persistence, and the replay ledger update verified in the Docker cohort.
...
**Verdict:** PASS - member removal stayed limited to a status-list revoke and CRL remained completely unchanged.
...
**Verdict:** PASS - Mesh Circle archive block, runtime gossip isolation, and Milestone-4 gossip health all verified intact in the Docker cohort.
...
**Verdict:** PASS - the Circle CRUD flow verified end-to-end in the Docker cohort.
...
**Verdict:** PASS - the Members API create, list, role-change, and remove flow verified.
...
**Verdict:** PASS - the Invites API mint, list, and revoke flow verified.
...
**Verdict:** PASS - join/preview, redeem, nodeB VC persistence, and the redeemed-ledger update all verified.
