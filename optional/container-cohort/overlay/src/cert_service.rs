...
fn local_platform_class() -> crate::platform::traits::PlatformClass {
    crate::platform::PlatformContext::global()
        .map(|ctx| ctx.platform_class())
        .unwrap_or_else(|| {
            if std::path::Path::new("/proc/device-tree/model").exists() {
                crate::platform::traits::PlatformClass::Real
            } else {
                crate::platform::traits::PlatformClass::Virtual
            }
        })
}
...
fn load_ca_bootstrap_identity_with_paths(
    ...
    let local_class = local_platform_class();
    if force_software_keys && local_class != crate::platform::traits::PlatformClass::Virtual {
        return Err(Status::failed_precondition(
            "CA software-key bootstrap is allowed only on the virtual dev platform",
        ));
    }
    ...
    if km.backend_name() != "SE050"
        && local_class != crate::platform::traits::PlatformClass::Virtual
    {
        return Err(Status::failed_precondition(
...
impl CertService for MyCertService {
    async fn request_certificate(
        ...
        let requester_platform_class = req.platform_class.trim().to_ascii_lowercase();
        let local_attestation_policy = load_config("/etc/sgx-guardian/config/nodeA.yaml")
            .or_else(|_| load_config("/etc/sgx-guardian/nodeA.yaml"))
            .ok()
            .map(|cfg| cfg.attestation_or_default())
            .unwrap_or_default();
        ...
        if !software_keys_forced() {
            if requester_platform_class.is_empty() {
                if !local_attestation_policy.accept_legacy_v1_quotes {
                    return Err(Status::permission_denied(
                        "CSR missing platform_class and legacy quote acceptance is disabled",
                    ));
                }
            } else if requester_platform_class == "virtual" {
                if !local_attestation_policy.accept_virtual_peers {
                    return Err(Status::permission_denied(
                        "CA is not configured to accept virtual-platform CSRs",
                    ));
                }
            } else if requester_platform_class != "real" {
                return Err(Status::permission_denied(format!(
                    "Production CA accepts only real-platform CSRs, got '{}'",
                    requester_platform_class
                )));
            }
        }
