...
struct QuotePolicy {
    accept_virtual_peers: bool,
    accept_legacy_v1_quotes: bool,
}
...
fn local_quote_policy() -> QuotePolicy {
    let node_id = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "nodeA".to_string());
    load_node_config_for_attestation(&node_id)
        .map(|cfg| {
            let att = cfg.attestation_or_default();
            QuotePolicy {
                accept_virtual_peers: att.accept_virtual_peers,
                accept_legacy_v1_quotes: att.accept_legacy_v1_quotes,
            }
        })
        .unwrap_or(QuotePolicy {
...
pub struct AttestationQuote {
    ...
    /// Signed platform class ("real" or "virtual").
    pub platform_class: String,
    /// Signed key backend ("SE050", "SoftHSM2", or "Software").
...
pub struct SignedQuote {
    ...
    /// Display-only signing backend for logs/UI.
    /// The authoritative backend is `AttestationQuote.key_backend`.
...
impl AttestationQuote {
    ...
    pub fn generate(challenge_nonce: &str, node_id: &str, km: &KeyManager) -> Result<Self> {
        let class = crate::platform::PlatformContext::global()
            .map(|ctx| ctx.platform_class())
            .unwrap_or_else(|| {
        ...
        Self::generate_for_platform(challenge_nonce, node_id, km, class)
    }
    ...
    pub fn generate_for_platform(
        ...
        let boot = crate::platform::PlatformContext::global()
            .map(|ctx| ctx.boot_chain_status())
            .unwrap_or_else(BootChainStatus::check);
        ...
        Ok(Self {
            ...
            pubkey_b64,
            platform_class: platform_class.as_str().to_string(),
            key_backend: km.backend_name().to_string(),
...
impl SignedQuote {
    ...
    pub fn verify(
        ...
        if quote.version < 2 {
            if !policy.accept_legacy_v1_quotes {
                result.reason = "Legacy v1 attestation quotes are disabled by local policy".into();
                return result;
            }
            eprintln!(
                "⚠️ Accepting legacy v1 quote from {} because accept_legacy_v1_quotes=true",
                quote.node_id
            );
        } else if !policy.accept_virtual_peers
            && (!quote.platform_class.eq_ignore_ascii_case("real") || quote.key_backend != "SE050")
        {
            result.reason = format!(
                "Quote rejected by local platform policy (platform_class={}, key_backend={})",
                quote.platform_class, quote.key_backend
            );
            return result;
        }
...
impl AttestationService {
    ...
    pub fn generate_quote(challenge_nonce: &str, km: &KeyManager) -> Result<SignedQuote> {
        if let Some(ctx) = crate::platform::PlatformContext::global() {
            return ctx.generate_quote(challenge_nonce, km);
        }
        let node_id = std::env::args().nth(1).unwrap_or_else(|| "nodeA".into());
...
pub async fn run(mut rx: Receiver<String>) -> Result<()> {
    ...
    if !peers_list.is_empty() {
        ...
        for peer in peers_list {
            ...
            match crate::vc::issue::load_runtime_key_manager(&node_id) {
                Ok(km) => {
    ...
    while let Some(peer) = rx.recv().await {
        ...
        match crate::vc::issue::load_runtime_key_manager(&node_id) {
            Ok(km) => {
                match AttestationService::mutual_attest(target_ip.clone(), attest_port, km.as_ref())
                    .await
                {
            ...
            Ok((mut socket, remote)) => {
                ...
                match AttestationService::verify_signed_evidence_with_peer_addr(
                    ...
                    Ok(true) => {
                        ...
                        let mut reply =
                            AttestationService::create_signed_evidence(km.as_ref(), &policy.yaml)?;
                        if let Ok(Some(vc)) = crate::vc::persistence::load_own_any() {
