...
fn build_signed_pcr_baseline(
    snapshot: &sgx_guardian_client::secure_element::pcr::PcrSnapshot,
    km: &KeyManager,
) -> Result<sgx_guardian_client::secure_element::pcr::PcrBaseline, String> {
    use sgx_guardian_client::secure_element::pcr::PcrBaseline;

    let created_at = chrono::Utc::now().to_rfc3339();
    let composite_bytes =
        hex::decode(&snapshot.composite_digest).map_err(|e| format!("decode composite: {}", e))?;
    let mut sign_input =
        Vec::with_capacity(composite_bytes.len() + created_at.len() + snapshot.device_uid.len());
    sign_input.extend_from_slice(&composite_bytes);
    sign_input.extend_from_slice(created_at.as_bytes());
    sign_input.extend_from_slice(snapshot.device_uid.as_bytes());
    let sign_hash = sha2::Sha256::digest(&sign_input);
    let signature = km
        .sign(&sign_hash)
        .map_err(|e| format!("sign baseline: {}", e))?;
    ...
    Ok(PcrBaseline {
        ...
        signing_public_key_sha256: Some(hex::encode(sha2::Sha256::digest(
            km.pubkey_der().unwrap_or_default(),
        ))),
        signature_format: Some(sgx_guardian_client::secure_element::pcr::signature_format(
            &signature,
        )
        .to_string()),
        created_at,
...
fn refresh_virtual_pcr_baseline(
    node_id: &str,
    baseline_path: &str,
    snapshot: &sgx_guardian_client::secure_element::pcr::PcrSnapshot,
    km: &KeyManager,
    ...
    let baseline = build_signed_pcr_baseline(snapshot, km)?;
    baseline.save(baseline_path)?;

    let pubkey = km
        .pubkey_der()
        .map_err(|e| format!("load baseline pubkey: {}", e))?;
    if !baseline.verify_signature(&pubkey) {
        return Err("newly written baseline failed signature verification".to_string());
    }

    println!(
        "  ♻️ Virtual PCR baseline refreshed for {} ({})",
        node_id, reason
    );
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let accept_virtual_peers_default = if std::env::var("SGX_PLATFORM_MODE")
    ...
    let current_node_config = {
        let config_path = format!("/etc/sgx-guardian/config/{}.yaml", node_id);
        let root_path = format!("/etc/sgx-guardian/{}.yaml", node_id);
        load_config(&config_path)
            .or_else(|_| load_config(&root_path))
            .map_err(|e| format!("Failed to load node config for PAL init: {}", e))?
    };
    let platform_ctx = match PlatformContext::init(current_node_config.clone()) {
        Ok(ctx) => ctx.install_global(),
        Err(e) => {
            eprintln!("❌ Platform initialization failed: {}", e);
            std::process::exit(1);
        }
    };
    if platform_ctx.is_virtual() {
        println!(
            "🧪 Platform mode: VIRTUAL ({}) — backend will stay isolated from production trust",
            platform_ctx.requested_mode()
        );
    } else {
        println!(
            "🔒 Platform mode: REAL ({}) on {}",
            platform_ctx.requested_mode(),
            platform_ctx.device_model()
        );
    }
    ...
    let km = match platform_ctx.initialize_key_manager(&node_id, &node_key_path) {
        Ok(km) => {
            if platform_ctx.is_real() && GATES.force_software_keys && GATES.allow_debug_soft_keys {
                println!("🔐 Debug software-key mode enabled on real hardware");
                log_audit(
                    &node_id,
                    AuditCategory::Identity,
                    AuditSeverity::Critical,
                    AuditAction::Loaded,
                    "Real hardware booted with SGX_FORCE_SOFTWARE_KEYS + SGX_ALLOW_DEBUG_SOFT_KEYS",
                );
                ...
                println!("🔐 Runtime key backend: {}", km.backend_name());
            }
            km
        }
        Err(e) => {
            log_audit(
                ...
                AuditAction::Failed,
                &format!(
                    "Platform key initialization failed — refusing to start: {}",
                    e
                ),
            );
            eprintln!(
                "❌ Refusing to start: {}. Runtime key initialization is fail-closed and will not silently fall back.",
                e
            );
            std::process::exit(1);
        }
    ...
    println!("\n  Verifying secure boot chain...");
    {
        ...
        let boot_status = if GATES.disable_secure_boot_check && platform_ctx.is_real() {
            tracing::warn!("Secure boot check disabled by SGX_DISABLE_SECURE_BOOT_CHECK");
            BootChainStatus::unknown()
        } else {
            let platform_ctx_boot = platform_ctx.clone();
            match tokio::time::timeout(
                std::time::Duration::from_secs(15),
                tokio::task::spawn_blocking(move || platform_ctx_boot.boot_chain_status()),
            )
            .await
            {
                Ok(Ok(s)) => s,
    ...
    let pcr_measurement_disabled = GATES.disable_pcr_measurement && platform_ctx.is_real();
    let mut local_pcr_trusted = pcr_measurement_disabled;
    // === PCR Measurement (ATT-003) ===
    println!("\n  Measuring platform integrity (PCR)...");
    if pcr_measurement_disabled {
        ...
        println!(
            "  PCR mode: {}",
            if platform_ctx.is_real() {
                "Hardware (board)"
            } else {
                "Virtual (provider-simulated)"
            }
        );

        let sources = platform_ctx.measurement_sources();
        ...
        let mut virtual_baseline_refresh_reason: Option<String> = None;
        if let Ok(baseline) = PcrBaseline::load(&baseline_path) {
            ...
            if baseline.schema_version != PCR_SCHEMA_VERSION {
                ...
                virtual_baseline_refresh_reason = Some(format!(
                    "schema changed from v{} to v{}",
                    baseline.schema_version, PCR_SCHEMA_VERSION
                ));
            } else {
                ...
                if !baseline.verify_signature(&pubkey) {
                    if baseline.key_version != snapshot.key_version {
                        ...
                        virtual_baseline_refresh_reason = Some(format!(
                            "DKP version changed from v{} to v{}",
                            baseline.key_version, snapshot.key_version
                        ));
                    } else {
                        eprintln!("  🔴 Baseline signature INVALID — possible tampering!");
                        virtual_baseline_refresh_reason =
                            Some("signature no longer matches current dev key".to_string());
                    }
                } else {
                    match snapshot.compare_baseline(&baseline) {
                        ...
                        Ok(mismatches) => {
                            ...
                            virtual_baseline_refresh_reason = Some(format!(
                                "stale dev baseline on PCRs {:?}",
                                mismatches
                            ));
                        }
                        Err(e) => {
                            eprintln!("  ⚠️ Baseline compare error: {}", e);
                            virtual_baseline_refresh_reason =
                                Some(format!("baseline compare error: {}", e));
                        }
                    }
                }
            }
            ...
            virtual_baseline_refresh_reason = Some("baseline missing".to_string());
        }

        if !local_pcr_trusted && platform_ctx.is_virtual() && snapshot.integrity_status != "FAIL" {
            if let Some(reason) = virtual_baseline_refresh_reason.as_deref() {
                if let Err(e) =
                    refresh_virtual_pcr_baseline(&node_id, &baseline_path, &snapshot, &km, reason)
                {
                    eprintln!("  ⚠️ Virtual baseline refresh failed: {}", e);
                } else {
                    local_pcr_trusted = true;
                }
            }
