...
impl KeyManager {
    ...
    #[cfg(feature = "virtual-platform")]
    pub fn init_virtual_pkcs11(node_id: &str, fallback_key_path: &str) -> Result<Self> {
        let material = crate::platform::virtual_hw::softhsm::ensure_virtual_dkp(node_id)?;
        let _ = crate::platform::virtual_hw::softhsm::ensure_named_key(node_id, "dik", 1)?;
        let rng = SystemRandom::new();
        let pkcs8_bytes = fs::read(&material.key_path)
            .map_err(|e| anyhow!("Read virtual key {}: {}", material.key_path, e))?;
        let fallback_key = Path::new(fallback_key_path);
        if let Some(parent) = fallback_key.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| anyhow!("create {}: {}", parent.display(), e))?;
        }
        if !fallback_key.exists() {
            write_private_key(fallback_key, &pkcs8_bytes)?;
        }
        let keypair =
            EcdsaKeyPair::from_pkcs8(&ECDSA_P256_SHA256_FIXED_SIGNING, &pkcs8_bytes, &rng)
                .map_err(|_| {
                    anyhow!("Failed to load virtual keypair from {}", material.key_path)
                })?;
        ...
        Ok(Self {
            ...
            backend: SigningBackend::Pkcs11 {
                ...
                raw_public_key: material.raw_public_key,
                public_key_spki_der: material.spki_public_key,
            },
        })
    }
    ...
    pub fn init_with_se050(
        ...
        match DkpManager::init(se_config, base_path) {
            ...
            Err(e) => Err(anyhow!(
                "SE050 DKP init failed: {} — fail-closed on hardware platform",
                e
            )),
        }
    }
    ...
    /// Returns the on-disk runtime export form used by DID/bootstrap helpers.
    /// Software keeps the historical raw-SEC1 export; hardware and virtual
    /// backends preserve their SPKI DER exports.
    pub fn runtime_public_key_export(&self) -> Result<Vec<u8>> {
        match &self.backend {
            SigningBackend::Software => self.pubkey_der(),
            #[cfg(feature = "secure-element")]
            SigningBackend::Hardware { .. } => {
                std::fs::read("/var/lib/sgx-guardian/keys/dkp_pub.der")
                    .map_err(|e| anyhow!("read hardware runtime pubkey export: {}", e))
            }
            #[cfg(feature = "virtual-platform")]
            SigningBackend::Pkcs11 {
                public_key_spki_der,
