...
pub fn read_device_uid_from_source(fallback: &str) -> String {
...
/// Platform-aware UID reader. When PAL is initialized it uses the active
/// provider (real SE050 UID or persisted virtual UID); otherwise it falls
/// back to the historical direct ssscli path.
pub fn read_device_uid(fallback: &str) -> String {
    if let Some(ctx) = crate::platform::PlatformContext::global() {
        if let Ok(uid) = ctx.device_uid() {
            return uid;
        }
    }
    read_device_uid_from_source(fallback)
}
