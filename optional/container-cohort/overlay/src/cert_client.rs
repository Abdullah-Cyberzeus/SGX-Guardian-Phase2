...
async fn try_request(
    ...
    let platform_class = crate::platform::PlatformContext::global()
        .map(|ctx| ctx.platform_class().as_str().to_string())
        .unwrap_or_else(|| {
    ...
    let attestation_quote_json = crate::vc::issue::load_runtime_key_manager(node_id)
        .ok()
        .and_then(|km| {
            crate::attestation_service::AttestationService::generate_quote(
                &format!("cert-bootstrap-{}", node_id),
                &km,
            )
            .ok()
            .map(|signed| signed.quote_json)
        })
        .unwrap_or_default();
