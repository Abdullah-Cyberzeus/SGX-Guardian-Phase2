...
use sgx_guardian_client::vault::{VAULT_BASE_ENV, VaultConfiguse axum::Extension;
...
async fn pwa_side_contacts_test_covers_member_roster_and_private_address_book() {
    ...
    assert_eq!(
        created_guardian.contact.owner_did.as_deref(),
    ...
    assert_eq!(
        created_bob.contact.owner_did.as_deref(),
        Some(alice_did.as_str())
    );
    ...
    assert_eq!(
        bob_saved_alice.contact.owner_did.as_deref(),
        Some(bob_did.as_str())
    );
    ...
    assert_eq!(fetched.owner_did.as_deref(), Some(alice_did.as_str()));
    ...
    assert_eq!(
        created_remote.contact.owner_did.as_deref(),
        Some(alice_did.as_str())
    );
