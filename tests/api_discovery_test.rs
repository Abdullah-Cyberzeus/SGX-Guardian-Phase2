...
async fn test_discovery_endpoints() {
    ...
    let res = client.get(&format!("{}/api/v1/discovery/devices", base_url)).send().await.unwrap();
    ...
    let res = client.get(&format!("{}/api/v1/discovery/unauthorized", base_url)).send().await.unwrap();
    assert_eq!(res.status(), reqwest::StatusCode::OK);
    let unauth: serde_json::Value = res.json().await.unwrap();
    assert!(unauth.as_array().unwrap().len() == 1);

    // 3. POST /api/v1/discovery/scan/now
    let res = client.post(&format!("{}/api/v1/discovery/scan/now", base_url)).send().await.unwrap();
    let _ = res.status();

    // 4. POST /api/v1/discovery/scan/stealth
    let res = client.post(&format!("{}/api/v1/discovery/scan/stealth", base_url)).send().await.unwrap();
    let _ = res.status();

    // 5. GET /api/v1/discovery/whitelist
    let res = client.get(&format!("{}/api/v1/discovery/whitelist", base_url)).send().await.unwrap();
    assert_eq!(res.status(), reqwest::StatusCode::OK);

    // 6. PUT /api/v1/discovery/whitelist
    let res = client.put(&format!("{}/api/v1/discovery/whitelist", base_url))
    ...
    let res = client.post(&format!("{}/api/v1/discovery/approve", base_url))
        .json(&serde_json::json!({
            "mac": "AA:BB:CC:11:22:33",
    ...
    let res = client.get(&format!("{}/api/v1/discovery/schedule", base_url)).send().await.unwrap();
    assert_eq!(res.status(), reqwest::StatusCode::OK);

    // 9. PUT /api/v1/discovery/schedule
    let res = client.put(&format!("{}/api/v1/discovery/schedule", base_url))
        .json(&serde_json::json!({
            "enabled": true,
            "timeout_secs": 600
        }))
