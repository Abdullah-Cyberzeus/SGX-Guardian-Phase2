...
fn should_block_rejects_info_severity() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(!Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::Info)));
}

#[test]
fn should_block_rejects_low_severity() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(!Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::Low)));
}

#[test]
fn should_block_rejects_medium_severity() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(!Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::Medium)));
}

#[test]
fn should_block_accepts_high_severity() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::High)));
}

#[test]
fn should_block_accepts_critical_severity() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::Critical)));
}

#[test]
fn should_block_rejects_alert_only_mode() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::AlertOnly, ..Default::default() };
    assert!(!Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::Critical)));
}

#[test]
fn should_block_rejects_when_disabled() {
    let cfg = SuricataConfig { enabled: false, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(!Blocker::should_block(&cfg, &alert("203.0.113.1", Severity::Critical)));
}
...
fn should_block_rejects_cidr_exempt() {
    ...
    assert!(!Blocker::should_block(&cfg, &alert("203.0.113.99", Severity::Critical)));
}
...
fn should_block_accepts_invalid_source_ip_when_not_exempt() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(Blocker::should_block(&cfg, &alert("bad-ip", Severity::Critical)));
}

#[test]
fn block_record_round_trips_json() {
    let record = sgx_guardian_client::threat::blocker::BlockRecord { ip: "203.0.113.1".into(), expires_at: 123 };
    assert_eq!(serde_json::from_value::<sgx_guardian_client::threat::blocker::BlockRecord>(serde_json::to_value(&record).unwrap()).unwrap(), record);
}

#[test]
fn block_attempt_equality_works() {
    let a = sgx_guardian_client::threat::blocker::BlockAttempt { blocked: true, reason: Some("r".into()) };
    assert_eq!(a, a.clone());
}

#[test]
fn block_attempt_debug_mentions_blocked() {
    let a = sgx_guardian_client::threat::blocker::BlockAttempt { blocked: false, reason: None };
...
fn load_block_records_missing_file_returns_empty() {
    ...
    assert!(sgx_guardian_client::threat::blocker::load_block_records(&dir.path().join("missing.json")).unwrap().is_empty());
}

#[test]
fn load_block_records_empty_file_returns_empty() {
    let file = tempfile::NamedTempFile::new().unwrap();
    assert!(sgx_guardian_client::threat::blocker::load_block_records(file.path()).unwrap().is_empty());
}
...
fn load_block_records_valid_json_loads() {
    ...
    assert_eq!(sgx_guardian_client::threat::blocker::load_block_records(file.path()).unwrap().len(), 1);
}

#[test]
fn should_block_ipv6_high_severity() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(Blocker::should_block(&cfg, &alert("2001:db8::1", Severity::High)));
}
...
fn should_block_ipv4_boundary_first_address() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(Blocker::should_block(&cfg, &alert("0.0.0.0", Severity::Critical)));
}

#[test]
fn should_block_ipv4_boundary_last_address() {
    let cfg = SuricataConfig { enabled: true, block_mode: BlockMode::InlineBlock, ..Default::default() };
    assert!(Blocker::should_block(&cfg, &alert("255.255.255.255", Severity::Critical)));
}
...
fn should_block_exact_exempt_takes_precedence_over_high_severity() {
    ...
    assert!(!Blocker::should_block(&cfg, &alert("198.51.100.8", Severity::High)));
}
...
fn load_block_records_preserves_multiple_records() {
    ...
    .unwrap();
    assert_eq!(sgx_guardian_client::threat::blocker::load_block_records(file.path()).unwrap().len(), 2);
}
