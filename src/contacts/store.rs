...
pub async fn list(path: &Path) -> Result<Vec<Contact>, ApiError> {
...
pub async fn get(path: &Path, did: &str) -> Result<Contact, ApiError> {
    let did = normalize_did(did)?;
    load(path)
        .await?
        .into_iter()
        .find(|contact| contact.did == did)
        .ok_or_else(|| ApiError::NotFound(format!("Contact not found for DID {}", did)))
}

pub async fn create(path: &Path, draft: ContactDraft) -> Result<Contact, ApiError> {
    let did = normalize_did(&draft.did)?;
    let lock = CONTACTS_LOCK.get_or_init(|| Mutex::new(()));
    let _guard = lock.lock().await;
    let mut contacts = load(path).await?;
    if contacts.iter().any(|contact| contact.did == did) {
...
pub async fn update(path: &Path, did: &str, patch: ContactPatch) -> Result<Contact, ApiError> {
    let did = normalize_did(did)?;
    let lock = CONTACTS_LOCK.get_or_init(|| Mutex::new(()));
    let _guard = lock.lock().await;
    let mut contacts = load(path).await?;
...
pub async fn delete(path: &Path, did: &str) -> Result<bool, ApiError> {
    let did = normalize_did(did)?;
    let lock = CONTACTS_LOCK.get_or_init(|| Mutex::new(()));
    let _guard = lock.lock().await;
    let mut contacts = load(path).await?;
    let before = contacts.len();
    contacts.retain(|contact| contact.did != did);
