...
// CHANGES FROM V1:
//
// In V1, main.rs took peer IPs from config for startup broadcast:
...
//   - Let p2p_discovery.rs handle config sync once a real peer is found
//   - No behavioral change in this file; only main.rs integration was simplified
//   - For manual broadcast, use broadcast_to_real_ip() to send directly to a
//     known reachable IP (not a config seed value)
// ================================================================
...
pub async fn broadcast_own_config_to_peers(
    ...
    peer_ips: &[String], // These IPs must be real reachable peer addresses.
) {
...
/// Sends config to one peer at a known reachable IP.
...
// p2p_discovery handles real broadcasts.
// This monitor updates local config and triggers future mDNS re-announcements.
pub async fn start_ip_monitor(
    node_id: String,
    config_path: String,
    peer_ips: Vec<String>, // V2: empty at startup, or known reachable peer IPs.
...
// ── IP Format Sanitizer ─────────────────────────────────────────
// This function validates the IP in a config file.
...
pub fn sanitize_config_ip_if_invalid(config_path: &str) -> Result<()> {
    ...
    // Keep the current value when the IP is already valid.
    if current_ip.parse::<std::net::Ipv4Addr>().is_ok() {
        return Ok(());
    }

    // Invalid format: reset to 0.0.0.0.
    println!(
