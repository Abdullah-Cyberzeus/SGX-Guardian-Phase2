...
//
// The issue was that OverlayPool state was stored on only one machine.
// If nodeA ran on PC1 and later moved to PC2, the .1 address could be
// reassigned, causing an IP conflict.
//
// This file maintains a JSON-based registry where:
//   - Each node name maps to a persistent overlay IP
//   - The Circle owner (nodeA) always gets .1
//   - An atomic counter tracks the next available IP
//   - File locking keeps concurrent writes safe
//
// SYNC STRATEGY:
//   - nodeA (CA) stores the registry under /var/lib/sgx-guardian/nebula/
//   - Member nodes register during certificate requests
//   - nodeA's registry is the master and is the only allocator
...
// It allows straightforward conversion from registry state to pool state.
