...
fn generate_remote_node_config(node_id: &str, overlay_ip: &str) -> String {
    let vps_public_ip = std::env::var("SGX_VPS_PUBLIC_IP").unwrap_or_else(|_| {
        eprintln!("⚠️  SGX_VPS_PUBLIC_IP not set — remote config will have placeholder");
        "VPS_PUBLIC_IP".to_string()
    });
    let vps_overlay_ip = std::env::var("SGX_VPS_OVERLAY_IP")
        .unwrap_or_else(|_| "192.168.100.10".to_string());
