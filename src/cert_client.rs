...
//!
//! Checks /var/lib/sgx-guardian/nebula/nodes/<node>.crt on startup.
//! If missing, sends gRPC request to nodeA's PLAINTEXT bootstrap port
//! and retries every 5 seconds.
//! Once cert appears (via gRPC response OR local file), stops retrying.
//!
//! Uses PLAINTEXT (http://) because this is a bootstrap operation —
//! the node doesn't have a trusted cert yet (that's the whole point).
//! CertService runs on a dedicated plaintext port (50061) on nodeA.
//!
//! NO rcgen. NO /member-certs. NO /cert-requests outside nebula.
//! NO std::process::exit(). NO crash. NO blocking threads.
