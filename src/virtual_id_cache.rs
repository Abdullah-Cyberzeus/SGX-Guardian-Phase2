...
impl RotationReason {
    ...
    pub fn parse_label(value: &str) -> Option<Self> {
        match value {
            ...
            "pcr_changed" => Some(RotationReason::PcrChanged),
            "policy_changed" => Some(RotationReason::PolicyChanged),
            "multiple_security_inputs" => Some(RotationReason::MultipleSecurityInputs),
            "unknown_input_change" => Some(RotationReason::UnknownInputChange),
            _ => None,
        }
    }
}
...
impl VirtualIdCache {
    ...
    pub fn seed_if_missing_sync(
        ...
        let mut cache = self.inner.write().expect("VID cache poisoned");
        let seeded = cache.entry(peer_did.to_string()).or_insert(cached).clone();
        drop(cache);

        if !peer_ip_hint.is_empty() {
            self.ip_to_did
                .write()
                .expect("ip_to_did map poisoned")
                .insert(peer_ip_hint.to_string(), peer_did.to_string());
        }
...
fn classify_dkp_change(prev: &CachedVid, ctx: &ObservationContext<'_>) -> Option<bool> {
    ...
    let vm_changed = !prev.dkp_verification_method_id.is_empty()
        && !ctx.new_dkp_verification_method_id.is_empty()
        && prev.dkp_verification_method_id != ctx.new_dkp_verification_method_id;
    let kid_changed =
        !prev.dkp_kid.is_empty() && !ctx.new_dkp_kid.is_empty() && prev.dkp_kid != ctx.new_dkp_kid;
...
mod tests {
    ...
    #[test]
    fn classify_dkp_rotation_when_previous_state_only_has_fingerprint() {
        let cache = VirtualIdCache::new();
        cache.seed_if_missing_sync(
            "did:guardian:a",
            CachedVid {
                vid_hex: "vid-1".to_string(),
                stable_component_hex: compute_stable_security_state_hex(
                    "did:guardian:a",
                    "",
                    "",
                    "dkp-old",
                ...
                dkp_verification_method_id: String::new(),
                dkp_kid: String::new(),
                dkp_pubkey_sha256_b16: "dkp-old".to_string(),
                pcr_composite_digest: "pcr-1".to_string(),
    ...
    #[test]
    fn nonce_refresh_updates_last_reason_to_session_refresh() {
        let cache = VirtualIdCache::new();
        let _ = cache.observe_rich(&ctx(
            "did:guardian:a",
            "vid-1",
    ...
    #[test]
    fn nonce_only_parses_legacy_and_new_labels() {
        assert_eq!(
            RotationReason::parse_label("session_refreshed"),
            Some(RotationReason::NonceOnly)
        );
        assert_eq!(
            RotationReason::parse_label("nonce_refreshed"),
            Some(RotationReason::NonceOnly)
        );
        assert_eq!(
            RotationReason::parse_label("nonce_only"),
            Some(RotationReason::NonceOnly)
        );
    }
}
