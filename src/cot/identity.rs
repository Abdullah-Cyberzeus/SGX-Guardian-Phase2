...
//
// Decouples device identity from IP/MAC addresses.
// Identity = SHA-256(public_key_bytes).
// All trust decisions reference this fingerprint, not IPs.
// ============================================================
...
pub struct DeviceIdentity {
    ...
    /// Human-readable label (e.g., "nodeA"). Optional.
    /// NOT used for trust decisions — only for logging/display.
...
mod tests {
    ...
    fn sample_key() -> Vec<u8> {
        ...
        key.extend_from_slice(&[0xAA; 32]); // x-coordinate
        key.extend_from_slice(&[0xBB; 32]); // y-coordinate
        key
    }
