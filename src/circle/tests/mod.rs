...
impl EnvGuard {
    fn new() -> Self {
        ...
        let vid_state_dir = td.path().join("virtual-id");
...
fn targeted_invite_acceptance_issues_vc_and_persists_circle_state() {
    ...
    save_local_owner(&owner, &owner_doc);
    let owner_mesh_vc = issue::ensure_owner_vc(&owner, &owner_km).expect("owner vc");
    ...
    // This host test models two Guardians in one process. Switch all
    // process-global identity/registry paths before persisting nodeB's view,
    // matching the per-device filesystem isolation used on real boards.
    let node_b_base = _env._td.path().join("node-b");
    std::env::set_var(CIRCLE_BASE_ENV, node_b_base.join("circles"));
    std::env::set_var(crate::vc::persistence::VC_BASE_ENV, node_b_base.join("vc"));
    std::env::set_var(DID_PATH_ENV, node_b_base.join("did.json"));
    std::env::set_var(SELF_DOC_PATH_ENV, node_b_base.join("did_doc.json"));
    std::env::set_var(PEERS_DOC_DIR_ENV, node_b_base.join("peers"));
    std::env::set_var(CA_AGGREGATE_PATH_ENV, node_b_base.join("aggregate.json"));
    std::env::set_var(
        VERSION_COUNTER_PATH_ENV,
        node_b_base.join("version_counter"),
    );
    save_local_owner(&member, &member_doc);
    crate::vc::persistence::save_own(&owner_mesh_vc).expect("nodeB stores mesh vc");
    crate::vc::persistence::save_own(&vc).expect("nodeB stores own vc");
    invite::save_received_invite(token.clone()).expect("nodeB stores invite locally");
...
async fn http_group_call_create_is_idempotent_on_retry() {
    ...
    crate::virtual_id::observe_runtime_virtual_id(crate::virtual_id::RuntimeVirtualIdInputs {
        ...
        state_path: None,
        did: owner.did.clone(),
        dkp_pubkey_der: km.pubkey_der().expect("runtime VirtualID public key"),
        dkp_version: 1,
        pcr_values: vec!["aa".repeat(32)],
        pcr_digest: "bb".repeat(32),
        policy_digest: "cc".repeat(32),
    })
    .expect("runtime VirtualID");
