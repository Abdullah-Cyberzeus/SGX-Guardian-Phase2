...
async fn collect_protected_networks() -> Vec<ipnet::IpNet> {
    ...
    for gw in default_gateway_ips().await {
        let gw_net = match gw {
            IpAddr::V4(v4) => ipnet::IpNet::V4(ipnet::Ipv4Net::new(v4, 32).expect("v4 /32")),
            IpAddr::V6(v6) => ipnet::IpNet::V6(ipnet::Ipv6Net::new(v6, 128).expect("v6 /128")),
        };
        nets.push(gw_net);
    }
